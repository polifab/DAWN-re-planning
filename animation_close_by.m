...
% The animations works by plotting the position of the spacecraft, the 
% closest "planet" and displaying the current day.
...
% init dawn model
stl_offset = [0; 0; 0];
p_dawn = patch(stlread('Dawn_19.stl'));
T_dawn = hgmat(eye(3)*scale_stl* scale_dawn, pos_dawn_init);
t_dawn = hgtransform('Parent',gca);
set(t_dawn,'Matrix',T_dawn);
set(p_dawn,'Parent',t_dawn);
set(p_dawn, 'facec', col_dawn_face);            % Set the face colour
