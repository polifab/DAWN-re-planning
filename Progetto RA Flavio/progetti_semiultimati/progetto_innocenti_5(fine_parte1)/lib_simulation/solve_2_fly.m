%calcolo velocità uscita navicella
f.tempo_volo = time_hyp(f);
vol2.temp_start = f.day_m+f.tempo_volo/24;
day_start2 = fix(vol2.temp_start);
...
f.Vinf_p = [f.Vinf_p(1,1); f.Vinf_p(2,1); f.Vinf_p(3,1)];                                                                      %velocità uscita soi rispetto frame sole

f.Vp_p = [vol2.datiSoi.dati_venere(1,2); vol2.datiSoi.dati_venere(2,2); vol2.datiSoi.dati_venere(3,2)];                    %velocità venere rispetto sole

f.Vs_p = f.Vinf_p+f.Vp_p;
vol2.vel_start = f.Vs_p;                                                                                                                         %velocità uscita navicella soi rispetto sole
...
E0 = atan(sqrt((1-vol2.e)/(1+vol2.e))*tan(vol2.t_anom0*0.5))*2;
vol2.mean_anom0 = E0-vol2.e*sin(E0);
if vol2.mean_anom0 < 0
    vol2.mean_anom0 = 2*pi + vol2.mean_anom0;
end
