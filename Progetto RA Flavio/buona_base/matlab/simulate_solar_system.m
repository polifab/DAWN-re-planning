function [mercurio, venere, terra, marte] = simulate_solar_system(mercurio,venere,terra,marte,Nl,teta,ris)
    ...
    for i= 554:900
        ...
        %%
        %codice per la rotta: dopo flyby -> ultimo ingresso su venere
        if i >= 554
            if on == 0
                plot_orbN = plot3(navi3.pos(:,1),navi3.pos(:,2),navi3.pos(:,3),'magenta');hold on;
                on = 1;
            end
            
            manom = mean_anom_t(navi3.mean_anom0,venere.u_sol,navi3.a,i-554+1);       %mean anomaly of venus from J2018
            [ecc_anom_ven, teta_nav] = kepler1(manom,navi3.e);
            pos_lettura = fix(teta_nav/ris)+1;
            
            %v_navicella = eulerT(navi.W,navi.w,navi.i_orb)*[-sqrt(venere.u_sol*1e-9/navi.p)*sin(teta_nav); sqrt(venere.u_sol*1e-9/navi.p)*(navi.e + cos(teta_nav)); 0];
            
            plot_N = plot3(navi3.pos(pos_lettura,1),navi3.pos(pos_lettura,2),navi3.pos(pos_lettura,3),'magenta .', 'MarkerSize', 10);hold on;
            pause(0.05);
            delete(plot_N);
        end
